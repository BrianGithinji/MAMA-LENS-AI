 ...
 * Powered by MediLink Health (medilinkhealth.co.ke)
 * Connects mothers to doctors and specialists via secure video consultations.
 */
import { useState } from "react";
import { motion } from "framer-motion";
import {
  Video, Mic, MessageCircle, ExternalLink, Shield,
  Globe, Cpu, Heart, ChevronRight, Phone,
} from "lucide-react";
...
const CONSULTATION_TYPES = [
  {
    ...
    description: "Face-to-face with a doctor via secure video call",
  ...
  },
  {
    ...
    description: "Audio-only call with a healthcare provider",
  ...
  },
  {
    ...
    description: "Text-based consultation at your own pace",
...
const FEATURES = [
  {
    ...
    description: "Connect with doctors and specialists worldwide, anytime",
  },
  {
    icon: Shield,
    title: "HIPAA & GDPR Compliant",
    description: "Military-grade encryption protects your health data",
  },
  {
    icon: Cpu,
    title: "AI Health Assistants",
    description: "Meet Lora & Josh — 24/7 AI-powered medical guidance",
  },
  {
    icon: Heart,
    title: "Maternal Specialists",
    description: "OB/GYN, midwives, and maternal-fetal medicine experts",
  },
];
...
export default function TelemedicinePage() {
  ...
  const openMediLink = (path: string = "") => {
    setLaunching(true);
    window.open(`${MEDILINK_URL}${path}`, "_blank", "noopener,noreferrer");
  ...
  return (
    <div className="min-h-screen bg-warm-50 pb-20">
      ...
      <div className="max-w-lg mx-auto px-4 -mt-8 space-y-4">
        ...
        {/* Main CTA — Launch MediLink */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="bg-white rounded-3xl shadow-card overflow-hidden"
        >
          {/* MediLink branding strip */}
          ...
          <div className="p-5">
            ...
            <div className="space-y-2 mb-4">
              {CONSULTATION_TYPES.map(({ icon: Icon, label, description, color, action }) => (
                <button
                  key={label}
                  onClick={() => openMediLink()}
            ...
            <p className="text-center text-gray-400 text-xs mt-3">
              Opens medilinkhealth.co.ke in a new tab
            </p>
          </div>
        </motion.div>
        ...
        >
          <a
            href="tel:999"
            className="flex items-center gap-4 bg-emergency-50 border border-emergency-200 rounded-3xl p-4 active:scale-95 transition-all"
          >
          ...
          <div className="space-y-3">
            {HOW_IT_WORKS.map(({ step, title, desc }) => (
              <div key={step} className="flex items-start gap-3">
                <div className="w-7 h-7 rounded-full bg-calm-100 text-calm-600 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">
                  {step}
                </div>
                <div>
        ...
        >
          ...
          <p className="text-calm-100 text-xs mb-4">
            Create a free account to access doctors, specialists, and AI health assistants worldwide.
          </p>
          <button
            onClick={() => openMediLink()}
